                                ...
                                if (data['latitude'] != null && data['longitude'] != null) ...[
                                  Builder(
                                    builder: (context) {
                                      ...
                                      final String mapUrl = data['googleMapsUrl'] ?? 
                                          "https://www.google.com/maps/search/?api=1&query=$lat,$lng";

                                      return InkWell(
                                        onTap: () async {
                                           // 1. Try Native GEO Intent
                                           // Format: geo:lat,lng?q=lat,lng
                                           ...
                                           try {
                                             ...
                                             debugPrint("Geo intent failed: $e");
                                           }
                                           
                                           // 2. Fallback to HTTPS URL
                                           final Uri httpsUri = Uri.parse(mapUrl);
                                           try {
                                              if (!await launchUrl(httpsUri, mode: LaunchMode.externalApplication)) {
                                                 if (context.mounted) {
                                                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Could not open maps")));
                                                 }
                                              }
                                           } catch (e) {
                                              debugPrint("Map URL launch failed: $e");
                                           }
                                        },
                                        child: Container(
                                            ...
                                            children: [
                                              ...
                                              Expanded(
                                                child: Column(
                                                  crossAxisAlignment: CrossAxisAlignment.start,
                                                  children: [
                                                      const Text("View Location on Map", style: TextStyle(color: Colors.blue, fontWeight: FontWeight.bold)),
                                                      Text(
                                                        mapUrl, 
                                  ...
                                  Container(
                                      ...
                                      child: Row(
                                        children: [
                                           ...
                                           const SizedBox(width: 12),
                                           // "Location unavailable — retrying…" as requested
                                           Expanded(child: Text("Location unavailable — retrying...", style: TextStyle(color: Colors.orange.shade800))),
                                        ],
                                      ),
                                  )
                             ],
